...
`expected-vowels.jsonl`には、`expected.jsonl`の生音素列から抽出したモデル非依存の母音評価単位を記録します。隣接する同一母音だけを一つの`vowel_run`として結合し、異なる母音または子音を挟む同一母音は別単位として保持します。

- `vowel_index`: 発話内で0から始まる母音評価単位の番号
- `source_phoneme_start`: 元の音素配列における開始位置
- `source_phoneme_end`: 元の音素配列における終了位置（この位置は含まない）
- `expected_units`: 結合した母音音素の数
- `is_long`: `expected_units`が2以上かどうか
- `phoneme_review_status`: 元のG2P結果に対する読み確認の状態

このファイルには時間境界を含めません。各アライナーを実行した後、この評価単位へモデルごとの開始・終了時刻を対応付けます。
